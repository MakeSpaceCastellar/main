...
final class TransactionalServiceCompilerPass implements CompilerPassInterface
{
    ...
    private function serviceDecorator(ContainerBuilder $container)
    {
        return function ($tags, $serviceToBeTransactional) use ($container) {
            ...
            $container->register(
                $serviceToBeTransactional . '.transactional',
                TransactionalWrapper::class
            )->addArgument(new Reference($tags[0][self::BY_TAG]))->addArgument(
                new Reference($this->domainEventPublisherTag)
            )->addArgument(new Reference($serviceToBeTransactional . '.transactional.inner'))->setDecoratedService(
                $serviceToBeTransactional
            )->setPublic(false);
        };
    }
